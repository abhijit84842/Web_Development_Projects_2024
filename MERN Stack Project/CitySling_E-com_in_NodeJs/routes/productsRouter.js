...
router.get("/men",isUserLoggedIn , async(req,res)=>{
  try{
    const products= await ProductModel.find()

    let menProducts = products.filter((items)=>{
      return items.category ==="men"
    })
    res.render("menproducts", {men:menProducts})
  }catch(err){
    console.log(err.message)
  }
...
router.get("/kids" , isUserLoggedIn ,async(req,res)=>{
  try{
    const products = await ProductModel.find()
    let kidsProducts= products.filter((items)=>{
      return items.category ==="kids"
    })
    res.render("kidsproducts" ,{kids:kidsProducts})
  }catch(err){
    console.log(err.message)
  }
